...
const uint8_t INDUCTIVE_PIN = D5;
// ===== LCD =====
hd44780_I2Cexp lcd;
...
const float   DETECTION_MAX_CM = 4.0;
...
void processBottleDetection(){
  ...
  // MIXED SENSOR LOGIC:
  // Capacitive: Active-HIGH (HIGH = plastic detected)
  // Inductive:  Active-LOW  (LOW = metal detected)
  bool capacitiveActive = (capStable == HIGH);      // HIGH = plastic detected
  bool inductiveActive  = (inductiveStable == LOW); // LOW = metal detected
  
  String sortResult;
  bool isAccepted = false;
  
  // METAL DETECTION: Inductive active = ALWAYS metal (highest priority)
  // Capacitive may also trigger on metal, so we ignore it here
  if (inductiveActive && ultrasonicDetected) {
    sortResult = "invalid";
    isAccepted = false;
  ...
  // PLASTIC DETECTION: Capacitive active + Inductive inactive + Object present
  else if (capacitiveActive && !inductiveActive && ultrasonicDetected) {
    sortResult = "plastic";
    isAccepted = true;
...
void debounceInputs(){
  ...
  // ULTRASONIC (Gated within 2cm - 7cm window)
  ...
  if (capRead != capLast){ capLast = capRead; capChangedAt = now; } // Fixed typo
...
void handleDetection(){
  ...
  // Start detection ONLY when ultrasonic traces an object (Ultrasonic-First Logic)
  if (!detectionActive){
    bool cooldownPassed = (now - lastProcessedAt) >= COOLDOWN_AFTER_PROCESS_MS;
    
    // STRICT GATING: Only trigger if Ultrasonic confirms object presence
